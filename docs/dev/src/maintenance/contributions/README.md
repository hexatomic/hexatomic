...
```
  addRepository(type:0,location:file{#58}/<SRC>/releng/org.corpus_tools.hexatomic.update/target/repository/,Local,enabled:true);\
  addRepository(type:1,location:file{#58}/<SRC>/releng/org.corpus_tools.hexatomic.update/target/repository/,Local,enabled:true);\
```
